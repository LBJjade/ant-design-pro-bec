...
export default class TableList extends PureComponent {
  ...
  handleSearch = (e) => {
    ...
    const {commentManage: { data }, dispatch, form } = this.props;
    ...
    form.validateFields((err, fieldsValue) => {
      ...
      dispatch({
        type: 'commentManage/requireQuery',
  ...
  handelbatchDelete = (row) => {
    this.props.dispatch({
      type: 'commentManage/batchRemoveDelete',
      payload: row,
  ...
  handleEditModalVisible = (flag, id, resourceId, commentBrief) => {
    this.setState({
      modalVisible: flag,
  ...
  handleAdd = (fields) => {
    this.props.dispatch({
      type: 'commentManage/storeComment',
      payload: fields,
    });
  ...
  handleEdit = (fields) => {
    ...
    this.props.dispatch({
      type: 'commentManage/coverComment',
  ...
  validateComment = (rule, value, callback) => {
    if (value === undefined || value === "") {
      ...
      this.props.dispatch({
        type: 'commentManage/exisComments',
        payload: { where: {commentBrief: value} },
      }).then(() => {
        if (this.props.comments.results === undefined) {
          callback();
          return;
        }
        if (this.props.comments.results.length > 0) {
  ...
  validateCommentNo = (rule, value, callback) => {
    if (value === undefined || value === "") {
      callback();
    } else {
      this.props.dispatch({
        type: 'commentManage/exisCommentNos',
  ...
  render() {
    ...
    const { selectedRows, modalVisible, resourceId, commentBrief } = this.state;
    ...
    const columns = [
      ...
      {
        title: '评论',
        dataIndex: 'commentBrief',
      },
      {
        title: '点赞数',
        dataIndex: 'praiseTime',
      },
      {
        title: '操作',
        dataIndex: 'objectId',
    ...
    const parentAddMethods = {
      ...
      validateCommentNo: this.validateCommentNo,
    };
            ...
            <div className={styles.tableListForm}>
              <Form onSubmit={this.handleSearch} layout="inline">
                <Row gutter={{ md: 8, lg: 24, xl: 48 }}>
                  ...
                  <Col md={8} sm={24}>
                    <FormItem label="评论">
                      {getFieldDecorator('commentBrief')(
                        <Select
                          placeholder="请选择"
                          style={{ width: '100%' }}
                        >
                          { data.results.length > 0 ? data.results.map(d => <SelectOption key={d.objectId} value={d.commentBrief}>{d.commentBrief}</SelectOption>) :
